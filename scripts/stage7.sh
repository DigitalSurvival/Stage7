...
do
  ...
  case "${a}" in
    ...
    -q|--quiet)
      if [ ${verbose} = 1 ]; then
        usage "The --quiet and --verbose options are mutually exclusive. You can only use one."
        exit 1
      fi
      quiet=1
      ;;
    -v|--verbose)
      if [ ${quiet} = 1 ]; then
        usage "The --quiet and --verbose options are mutually exclusive. You can only choose one."
        exit 1
      fi
      verbose=1
      ;;
...
if [ "${install_mode}" != "chroot" ]; then
  runstep configure_bootloader "Configuring the bootloader..."
fi
